...
def Test_phase(model, args, k):
    ...
    for i in range(TEST_SIZE):
        for episode in test_loader:
            ...
            emb_total = model(data)
            emb_shot = emb_total[:25]
            emb_query = emb_total[25:]
              
            size = emb_shot.size()           
            
            c = torch.zeros(5,size[1]).cuda()
            for num_k in range(5):
                c[num_k] = torch.mean(emb_shot[5*num_k:5*(num_k+1)],0)
            
            logits = square_euclidean_metric(emb_query,c)
...
def train(args):
    ...
    if args.restore_ckpt is not None:
        del_name = len('module.')
        dictionay = torch.load(args.restore_ckpt)
        new_dictionay = OrderedDict()
        
        for key, value in dictionay.items():
            dict_name = key[del_name:] 
            new_dictionay[dict_name] = value
        
        model.load_state_dict(new_dictionay)
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            labels_new = labels_new.expand(args.query, 5).long()
            
            loss = F.log_softmax(-logits,dim=1)
            loss = torch.gather(loss,1,labels_new) *-1
            loss = loss.mean()
            

            """ TODO 2 END """
            acc = count_acc(logits, labels)
            #print('iteration : ', i ,'loss : ', loss.item(), 'acc : ', acc )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dpath', '--d', default='/data/CUB_200_2011/CUB_200_2011', type=str,
